...
class OrdersController extends Controller
{
   ...
   //cancel order return money
    ...
    public function orderDetails($id){
      ...
      public function orderDetailsNotification($id){
         
        $notification=AdminNotification::where('id', $id)->first(); 
        $order_details = Order::where('id',$notification->order_id)->first();
        $order_includes = OrderInclude::where('order_id',$notification->order_id)->get();
        $order_additionals = OrderAdditional::where('order_id',$notification->order_id)->get();

        // admin notification
       // $notification = AdminNotification::where('id', $notification->id)->first();
        if (!empty($notification)){
            if ($notification->status == 0){
                AdminNotification::where('id', $notification->id)->update(['status' => 1]);
            }
        }
