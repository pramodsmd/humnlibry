...
class ServiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Service::with(['category','subcategory'])->select('*')->orderBy('id', 'desc')->latest()->get();
  ...
  public function viewServiceDetailsfromNotifcation($id)
    {        
             $service_id=AdminNotification::where('id',$id)->first()->service_id??'';
            $service = Service::with('serviceInclude', 'serviceAdditional', 'serviceBenifit')->where('id', $service_id)->first();
            $service_includes = Serviceinclude::where('service_id', $service_id)->get();
            $service_additionals = Serviceadditional::where('service_id', $service_id)->get();
            $service_benifits = Servicebenifit::where('service_id', $service_id)->get();
            $service_faqs = OnlineServiceFaq::where('service_id', $service_id)->get();
            $seller_since = User::select('created_at')->where('id', $service->seller_id)->where('user_status', 1)->first();
            
            AdminNotification::where('id',$id)->update(['status' =>1]);
    ...
    public function service_create_status_settings_update(Request $request)
    {
      ...
      public function service_edit_status_settings_update(Request $request)
    {
        update_static_option('service_update_status_settings',$request->service_update_status_settings);
        return redirect()->back()->with(FlashMsg::item_new('Update Success'));
