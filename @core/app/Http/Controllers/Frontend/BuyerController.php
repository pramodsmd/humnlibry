...
class BuyerController extends Controller
{
    ...
    public function orderDetails($id=null)
    {
      ...
      public function orderDetailsfromNotification($id=null)
    {
        ...
        $order_details = Order::with('seller')
            ->where('id',$notification_id->order_id)
            ->where('buyer_id',Auth::guard('web')->user()->id)->first();
        ...
        if(!is_null($order_details)){
            ...
            DB::table('notifications')->where('id',$id)->update(['buyer_status'=>$currentDate]);
            return view('frontend.user.buyer.order.order-details', compact('order_details','order_includes','order_additionals','order_declines_history'));
        }
        abort(404);
    }
    ...
    public function orderCompleteRequestApprove($id=null)
    {
        ...
        toastr_success(__('booking complete request successfully approved.'));
        ...
        $order_message = __('Booking Complete request is approved');
        $seller->notify(new OrderNotification($id,$orderDetails->service_id,$orderDetails->seller_id, $orderDetails->buyer_id,$order_message));
        AdminNotification::create(['order_id' => $id,'type'=>'order_approved']);
    ...
    {   
        ...
        toastr_success(__('Booking successfully cancelled.'));
        
        $orderDetails  = Order::where('id',$id)->first();
        $seller = User::where('id',$orderDetails->seller_id)->first();
        $order_message = __('Booking Status Updated');
        $seller->notify(new OrderNotification($orderDetails->id,$orderDetails->service_id,$orderDetails->seller_id, $orderDetails->buyer_id,$order_message));
        AdminNotification::create(['order_id' => $orderDetails->order_id,'type'=>'order_cancelled']);
        return redirect()->back();
    ...
    public function orderCompleteRequestDecline(Request $request)
    {
        ...
        try {
            $message_body_admin = __('A customer has been decline a request to complete a booking. Booking ID #'). $request->order_id.'</br>';
            $message_body_seller = __('Your request to complete a booking has been decline by the customer. Booking ID #'). $request->order_id.'</br>';
        ...
        $order_message = __('Booking Complete request is decline');
        $seller->notify(new OrderNotification($orderDetails->id,$orderDetails->service_id,$orderDetails->seller_id, $orderDetails->buyer_id,$order_message));
        AdminNotification::create(['order_id' => $request->order_id,'type'=>'order_decline']);
        ...
        toastr_success(__('Booking complete request decline successfully'));
        return back();
    }
...
//            event(new SupportMessage($ticket_info));
    ...
    public function allNotification(){
         $notifications= DB::table('notifications')->where('buyer_status',null)->latest()->paginate(20);
        return view('frontend.user.buyer.notification.all-notification',compact('notifications'));
    }
