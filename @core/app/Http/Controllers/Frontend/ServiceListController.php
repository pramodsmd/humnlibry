...
class ServiceListController extends Controller
{
    ...
    { 
        ...
        if (Auth::guard('web')->check() && Auth::guard('web')->user()->type === 0){
            \Toastr::error(__('seller are not allowed to place service booking'));
            return back();
        }
        ...
        if($request->is_service_online_ != 1 && Auth::guard('web')->check() && Auth::guard('web')->user()->user_type == 1){
            ...
            if(Auth::guard('web')->check() && Auth::guard('web')->user()->user_type == 1 ){
               ...
               if( get_static_option('order_create_settings') !== 'anyone'){
                    toastr_error(__('You must login as a customer to create a booking.'));
        ...
        if($order_create !=''){
            SupportTicket::create([
                'title' => 'New Booking',
                'subject' => 'Booking Created By '.$request->name,
                'status' => 'open',
                'priority' => 'high',
        ...
        $buyer_id = Auth::guard('web')->check() ? Auth::guard('web')->user()->id : NULL;
        $order_message = __('You have a new booking');
        ...
        $description = sprintf(__('Booking id #%1$d Email: %2$s, Name: %3$s'),$last_order_id,$user_email,$user_name);

        //todo: check payment gateway is wallet or not || where payment gateway is null  then it will work for wallet
            ...
            if ($request->selected_payment_gateway == null) {
                ...
                if(!empty($wallet_balance)){
                    if($wallet_balance->balance >= $order_details->total){
                        ...
                        try {
                            $message_for_buyer = get_static_option('new_order_buyer_message') ?? __('You have successfully placed a booking #');
                            $message_for_seller_admin = get_static_option('new_order_admin_seller_message') ?? __('You have a new booking #');
                        ...
                        toastr_warning('Your wallet has '.float_amount_with_currency_symbol($shortage_balance).' shortage to booking this service. Please Credit your wallet first and try again.');
                        return back();
                    }
        ...
        if ($request->selected_payment_gateway === 'cash_on_delivery') {
            ...
            //Send order email to buyer for cash on delivery
            try {
                $message_for_buyer = get_static_option('new_order_buyer_message') ?? __('You have successfully placed an booking #');
                $message_for_seller_admin = get_static_option('new_order_admin_seller_message') ?? __('You have a new booking #');
                Mail::to($order_details->email)->send(new OrderMail(strip_tags($message_for_buyer).$order_details->id,$order_details));
                Mail::to($seller->email)->send(new OrderMail(strip_tags($message_for_seller_admin).$order_details->id,$order_details));
                Mail::to(get_static_option('site_global_email'))->send(new OrderMail(strip_tags($message_for_seller_admin).$order_details->id,$order_details));
