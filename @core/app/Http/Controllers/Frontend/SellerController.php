...
class SellerController extends Controller
{
    ...
    public function pendingOrders(Request $request)
    {
        if(!empty($request->order_id || $request->order_date)){
            $order_query = Order::with('service')->where('seller_id', Auth::guard('web')->user()->id)->where('status',0)->latest();
    ...
    public function orderDelete($id=null)
    {
        ...
        if($order->payment_status == 'pending' || $order->payment_status == ''){
            ...
            toastr_error(__('Booking Delete Success---'));
        }else{
            toastr_error(__('Booking Can Not be Deleted Due to Payment Status Complete---'));
        }
        return redirect()->back();
    }
    ...
    public function orderDetails($id=null)
    {
      ...
      public function orderDetailsNotification($id=null)
    {  
        $notifications= DB::table('notifications')->where('id',$id)->first()->data??'';
        $notification_id=json_decode($notifications);
        // dd($notification_id);
        $order_details = Order::where('id',$notification_id->order_id)->where('seller_id',Auth::guard('web')->user()->id)->first()??'';
        $order_declines_history = OrderCompleteDecline::where('order_id',$notification_id->order_id)->latest()->get();
        if(!empty($order_details)){
            $order_includes = OrderInclude::where('order_id',$notification_id->order_id)->get();
            $order_additionals = OrderAdditional::where('order_id',$notification_id->order_id)->get();
            foreach(Auth::guard('web')->user()->unreadNotifications()->where('data->order_message' , 'You have a new order')->get() as $notification){
                if($order_details->id == $notification_id->order_id){
                      $timestamp = time();
                      $currentDate = gmdate('Y-m-d h:i:s', $timestamp);
                       DB::table('notifications')->where('id',$id)->update(['read_at'=>$currentDate]);
                    // $Notification = Auth::guard('web')->user()->Notifications->find($notification_id->id);
                    // if($Notification){
                    //     $Notification->markAsRead();
                    // }
                    return view('frontend.user.seller.order.order-details', compact('order_details','order_includes','order_additionals','order_declines_history'));
        ...
        if($payment_status->status !=2){
            if($payment_status->payment_status =='complete'){
                ...
                if($request->status==2){
                    ...
                    try {
                        $message_body_buyer =__('Hello, ').$payment_status->name. __(' A new request is created for complete booking.').'</br>' . ' <span class="verify-code">'.__('Booking ID is: ') . $payment_status->id. '</span>';
                        $message_body_admin =__('Hello Admin A new request is created for complete a booking.').'</br>' . ' <span class="verify-code">'.__('Booking ID is: ') . $payment_status->id. '</span>';
                        Mail::to($payment_status->email)->send(new BasicMail([
                            'subject' => __('New Request For Complete a Booking'),
                    ...
                    $order_message = __('A new request is created for complete booking');
                   // seller buyer notification
                    $seller->notify(new OrderNotification($order_details->id,$order_details->service_id, $order_details->seller_id, $order_details->buyer_id,$order_message));
                    AdminNotification::create(['order_id' => $order_details->id,'type'=>'order_complete_request']);
...
//            event(new SupportMessage($ticket_info));
    ...
    public function createPayoutRequest(Request $request)
    {
        ...
        //         'subject' => 'New Payout Request By '.$request->name,
        //         'status' => 'open',
        //         'priority' => 'high',
        //         'seller_id' => Auth::guard('web')->user()->id,,
        //     ]);
    ...
    public function addNewTicket(Request $request,$id=null)
    {
        if($request->isMethod('post')){
            ...
            if($buyer){
                $order_ticcket_message = __('You have a new booking ticket');
            ...
            try {
                ...
                Mail::to(get_static_option('site_global_email'))->send(new BasicMail([
                    'subject' => get_static_option('order_ticket_subject') ?? __('New Booking Ticket'),
                    'message' => $message
                ]));
                Mail::to($buyer->email)->send(new BasicMail([
                    'subject' => get_static_option('seller_order_ticket_subject') ?? __('New booking Ticket'),
                    'message' => $message
                ]));
            } catch (\Exception $e) {
                return redirect()->back()->with(FlashMsg::item_new($e->getMessage()));
        ...
        if ($orderDetails->payment_status === 'complete'){
            ...
            if(!empty($service_details_for_book)){
                ...
                try {
                    ...
                    $message .= __('your have added extra service in your booking #').$orderDetails->id;
                    $message .= '</p>';
                    Mail::to($seller_details->email)->send(new BasicMail([
